...
def run_passivator_xrd(
    *,
    seed: int | None = None,
    verbose: bool = True,
) -> None:
    """
    Run Passivator XRD pipeline.
    ...
    """
    if verbose:
        print("▶ Running Passivator XRD pipeline...")
...
QUESTION = (
    "How does the peak intensity and residual stress of low dimensional perovskite characteristic peaks around 4 ° "
    "change in XRD characterization when passivating agents are used for surface treatment of perovskite?"
)

# Pair definition space (for passivators)
FORMULA_COLS = [
    "Formula PVK",
...
# Metrics used by filtering rule (rule from your passivator_pairs_evaluation.py)
INTENSITY_COL = "xrd_intensity_4"
STRESS_COL = "xrd_Stress"
PCE_COL = "PCE"
METRIC_COLS = [INTENSITY_COL, STRESS_COL, PCE_COL]
            ...
            for j in range(i + 1, len(values)):
                ...
                for _, row1 in a.iterrows():
                    for _, row2 in b.iterrows():
                        ...
                        # canonical ordering for de-duplication
...
# -------------------------
# 3) Pair filtering rule (passivator)
# -------------------------
def rule_xrd4_stress_case(
    row1: pd.Series, row2: pd.Series,
    intensity_col: str, stress_col: str, pce_col: str
) -> Optional[str]:
    """
    Rule aligned with passivator_pairs_evaluation.py:
      Case row1:
        (i1 < i2) AND (stress1 > stress2) AND (pce1 < pce2)
      Case row2:
        (i2 < i1) AND (stress2 > stress1) AND (pce2 < pce1)

    Return "row1"/"row2" indicating which row satisfies the condition.
    """
    i1, st1, p1 = row1[intensity_col], row1[stress_col], row1[pce_col]
    i2, st2, p2 = row2[intensity_col], row2[stress_col], row2[pce_col]

    if any(pd.isna(v) for v in (i1, i2, st1, st2, p1, p2)):
        return None

    if (i1 < i2) and (st1 > st2) and (p1 < p2):
        return "row1"
    if (i2 < i1) and (st2 > st1) and (p2 < p1):
        return "row2"
    return None
...
# -------------------------
# 4) Text generation via templates_lib (same template mechanism you used)
# -------------------------
...
def build_templates_for_row(row: pd.Series, T: Dict[str, Any]) -> Dict[str, str]:
    ...
    # Additive templates (kept as optional because your table may include it)
...
def generate_output_text(row: pd.Series, templates: Dict[str, str]) -> str:
    ...
    # XRD stress / low-dim peak around 4° (the key one for your passivator question)
...
# -------------------------
# 6) Build JSON records (Passivator, not Additive)
# -------------------------
def pairs_to_records(
    ...
    for _, pr in pairs_df.iterrows():
        ...
        # control = the row that satisfies the condition_case
        ...
        ctrl_pas = _collect_materials(row_ctrl, ["Formula Passivator 1", "Formula Passivator 2", "Formula Passivator 3"])
        tgt_pas = _collect_materials(row_tgt, ["Formula Passivator 1", "Formula Passivator 2", "Formula Passivator 3"])
        ...
        rec = {
            ...
            "Passivator": ordered_union(ctrl_pas, tgt_pas),
        }
        records.append(rec)
    return records
...
def main() -> None:
    ...
    pairs_formula_f = evaluate_pairs(
        ...
        intensity_col=INTENSITY_COL, stress_col=STRESS_COL, pce_col=PCE_COL,
    )
