...
└── src/
    ...
    │   ├── __init__.py
    │   ├── edge_mechanism_analyzer.py
    │   ├── edge_report_generator.py
...
2. **Initialize Database**  
   Run SQL script to create tables (skip if already executed by other layers). The input data (matching pairs) is generated by the Learning layer.
   ```bash
...
4. **Run Program**  
   ```bash
   python main.py generating --report variable          
   python main.py generating --report characterisation   
   python main.py generating --report edge             
   ```
