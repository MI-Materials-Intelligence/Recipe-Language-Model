...
MAPPINGS, COL_VOC, SCALER_VOC, MODEL_VOC = load_model_data("voc")
...
def get_voc(fp_params_initial: dict, task: str = "None") -> float:
    """
    Predict Voc based on recipe parameters.
    ...
    x_new = df_fp_params[COL_VOC].replace('', np.nan).fillna(0)
    x_new_std = SCALER_VOC.transform(x_new)
    y_new_pred_voc = MODEL_VOC.predict(x_new_std)[0]

    return float(y_new_pred_voc)
